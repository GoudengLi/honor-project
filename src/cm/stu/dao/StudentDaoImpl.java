...
public class StudentDaoImpl implements StudentDao{
    ...
    @Override
    public List<Person> etSearchTeacher(String matchText) {
    ...
    public List<StudentAnswer> goMyTeaTask(String userAccount, String teacherAccount) {
        String sql="SELECT s.*,p.`userName`,`taskName` FROM `studenttask` s INNER JOIN person p ON" +
                " s.`studentAccount`=p.`userAccount` INNER JOIN task t ON  s.`taskAccount`=t.`taskAccount` WHERE (s.`studentAccount`='"+userAccount+"' AND t.`teacherAccount`='"+teacherAccount+"');";
    ...
    public void addComment(String userName, int userIdentify, String userAccount, String taskName,String comment) {
        //`taskAccount``userAccount``userIdentify``comments``userName`
        String sql="INSERT INTO taskcomments values('"+taskName+"','"+userAccount+"','"+userIdentify+"','"+comment+"','"+userName+"') ";
        Deal.deal(sql);
    }

    @Override
    public void doAnswer(String userAccount, String taskAccount, String answer) {
        //UPDATE studenttask SET `studentAnswer`='',`isFinish`=1 WHERE `taskAccount`='' AND `studentAccount`=''
