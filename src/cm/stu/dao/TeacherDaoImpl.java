...
public class TeacherDaoImpl implements TeacherDao{
    ...
    public List AllStudentsByTeacherAccount(String userAccount) {
        String sql="SELECT * FROM `person` WHERE userAccount IN" +
                "(SELECT userAccount FROM `userteacher` WHERE teacherAccount='"+userAccount+"')";
    ...
    public List<StudentAnswer> getTaskDetail(String taskAccount) {
        //SELECT s.*,p.`userName`,`taskName` FROM `studenttask` s INNER JOIN person p ON s.`studentAccount`=p.`userAccount` INNER JOIN task t ON  s.`taskAccount`=t.`taskAccount` WHERE t.`teacherAccount`=''
    ...
    @Override
    //INSERT INTO `studenttask`(`taskAccount`,`studentAccount`,`studentAnswer`,`isFinish`,`Score`)
    //SELECT '121' ,B.C,NULL,0,0 FROM (SELECT `userAccount` C FROM `userteacher` WHERE `teacherAccount`='25555') AS B
    public void AddTask(Task task) {
        ...
        sql="INSERT INTO `studenttask`(`taskAccount`,`studentAccount`,`studentAnswer`,`isFinish`,`Score`)" +
                "SELECT '"+task.getTaskAccount()+"' ,B.C,NULL,0,0 FROM (SELECT `userAccount` C FROM `userteacher` WHERE `teacherAccount`='"+task.getTeacherAccount()+"') AS B";
        Deal.deal(sql);
    }
    ...
    public List getSearchStudentByTeacherAccount(String userAccount,String matchText) {
        String sql="SELECT * FROM person WHERE userAccount IN (SELECT `userAccount` FROM `userteacher` " +
                "WHERE teacherAccount='"+userAccount+"') AND (`userAccount`='"+matchText+"' OR `userName`" +
                " LIKE'"+matchText+"' OR userIdCard='"+matchText+"')";
        return Deal.getAllPerson(sql);
    }
    ...
    public void deleteThisPersonByTeacher(String studentAccount, String userAccount) {
        ...
        sql="DELETE FROM `studenttask` WHERE `taskAccount` IN(SELECT `taskAccount` FROM `task` " +
                "WHERE `teacherAccount`='"+userAccount+"') AND studentAccount='"+studentAccount+"'";
        Deal.deal(sql);
    }
}
